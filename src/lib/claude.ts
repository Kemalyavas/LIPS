...
export async function generateComment(
  ...
  // Include 3 real examples from the voice guide
  for (const ex of DANIELS_COMMENT_EXAMPLES.slice(0, 3)) {
    fewShotSection += `\nTopic: ${ex.topic}\nAI wrote: "${ex.aiDraft.substring(0, 250)}..."\nDaniel changed it to: "${ex.danielsVersion.substring(0, 250)}..."\n`;
  }
  ...
  if (learningPairs.length > 0) {
    ...
    for (const pair of learningPairs.slice(0, 5)) {
      fewShotSection += `\nAI wrote: "${pair.aiDraft.substring(0, 300)}"\nDaniel changed it to: "${pair.danielsVersion.substring(0, 300)}"\n`;
    }
  }
...
Write the comment now. Two to five sentences only. Sound like a real human in conversation, not a generated comment.`;
